...
import type { StoppableServer } from './FastMcpStarter';
import {
  Server as McpServer,
    ...
    const httpServer = createHttpServer(async (req: any, res: any) => {
      ...
      if (req.method === 'GET' && url.pathname === endpoint) {
        ...
        try {
          ...
          logger.error('Failed to establish SSE transport', { error } as any);
          res.end();
        }
        return;
      }
