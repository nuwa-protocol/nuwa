...
function acquire(server: McpServer, opts?: UseNuwaMCPOptions): void {
  ...
  if (GLOBAL) {
    if (GLOBAL.server === server) {
      GLOBAL.refCount += 1;
      if (debug || GLOBAL.debug) console.debug('[useNuwaMCP] Reusing existing MCP connection');
      return;
    }
 ...
 */
export function useNuwaMCP(server: McpServer, options?: UseNuwaMCPOptions): void {
  useEffect(() => {
    if (!server) return;
