...
If you need a zero-code DIDAuth flow for remote agents, use the `nuwa-id` CLI:
...
See the end-to-end guide: [DIDAuth for Remote Agents](/guides/did-auth-agent).
For backend verification middleware, see [DIDAuth for Backend Services](/guides/did-auth-server).
