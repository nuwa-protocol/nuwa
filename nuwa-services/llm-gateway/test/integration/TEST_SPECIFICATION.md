...
This document outlines the comprehensive test cases that should be implemented for each LLM provider integration. Use this as a checklist when adding support for a new provider.
...
**Note**: Error handling should be tested separately (e.g., in unit tests with mocked responses) to avoid consuming API quota and triggering provider security alerts.
...
Tests for basic chat completion functionality, both streaming and non-streaming modes.
...
- Response should include usage information (prompt tokens, completion tokens, total tokens)
- Response should include cost information
- Duration should be reasonable (< 30 seconds typically)
- Token counts should be consistent (totalTokens = promptTokens + completionTokens)
...
**Purpose**: Verify usage statistics are correctly extracted from non-streaming responses
...
**Purpose**: Verify usage statistics are extracted from streaming responses when available
...
**Test Requirements**:
- Use `pricingRegistry.getProviderPricing(provider, model)` to get pricing from configuration
- Calculate expected cost based on configuration pricing, not hardcoded values
- Verify actual cost is close to expected (within reasonable tolerance, e.g., 30%)
...
Tests for proper formatting and preparation of requests before sending to provider.

#### 4.1 Non-Streaming Request Preparation
**Purpose**: Verify request data is correctly prepared for non-streaming requests
...
Each provider should have a corresponding test utility class that extends `BaseProviderTestUtils`:
...
**Note**: Use `provider.getTestModels()` instead of static model lists to get provider-recommended test models.
...
- Accept both success and known unavailability conditions (model not configured, insufficient credits)
...
- Log when special conditions occur (unavailable models, configuration issues, etc.)
...
- [ ] Verified usage extraction works correctly (prompt tokens, completion tokens, total tokens)
- [ ] Verified cost calculation works correctly (from provider or gateway pricing)
