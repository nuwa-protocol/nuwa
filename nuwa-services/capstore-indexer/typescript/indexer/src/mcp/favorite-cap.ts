...
async function favoriteCap(
  { capId, action }: { capId: string; action: 'add' | 'remove' | 'isFavorite' },
  context: any
) {
  try {
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              code: 404,
              error:
                result.error || action === 'add'
                  ? 'Failed to add to favorite cap'
                  : 'Failed to remove from favorite cap',
            } as Result),
          },
        ],
      };
